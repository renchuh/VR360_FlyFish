...
public class FlockUnit : MonoBehaviour
{
	...
	[SerializeField] private float jumpForce;  // Added jump force for flying fish behavior
	[SerializeField] private float surfaceHeight; // Added surface height to detect when fish should jump
	[SerializeField] private float timeBetweenJumps; // Added time between jumps
	...
	public void MoveUnit()
	{
		...
		HandleJump();  // Added method to handle jumping
	}

	private void HandleJump()
	{
		if (myTransform.position.y >= surfaceHeight && !isJumping && Time.time > lastJumpTime + timeBetweenJumps)
		{
			isJumping = true;
		...
		if (isJumping && myTransform.position.y <= surfaceHeight)
		{
			isJumping = false;
		}
	}
